...
install_all() {
    ...
    rollback=1
    trap 'status=$?; trap - EXIT HUP INT TERM; if [ "$rollback" = 1 ]; then restore_pair "$backup_dir" || true; fi; exit "$status"' EXIT
...
remove_all() {
    ...
    [ -d "$original_backup/source" ] && [ -d "$original_backup/user" ] || {
    ...
    rollback=1
    trap 'status=$?; trap - EXIT HUP INT TERM; if [ "$rollback" = 1 ]; then restore_pair "$safety_backup" || true; fi; exit "$status"' EXIT
    trap 'exit 130' HUP INT TERM
