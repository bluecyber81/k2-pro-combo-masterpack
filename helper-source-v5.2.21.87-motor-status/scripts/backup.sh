...
backup_config() {
    ...
    [ -f "$SCRIPT_DIR/spoolman_cfs_map.json" ] && cp -a "$SCRIPT_DIR/spoolman_cfs_map.json" "$TMP_DIR/helper/" 2>/dev/null
    [ -d "$SCRIPT_DIR/state" ] && cp -a "$SCRIPT_DIR/state" "$TMP_DIR/helper/" 2>/dev/null
    [ -d "$SCRIPT_DIR/reports" ] && cp -a "$SCRIPT_DIR/reports" "$TMP_DIR/helper/" 2>/dev/null
    for f in \
        /etc/rc.d/S55klipper /etc/rc.d/S56moonraker /etc/rc.d/S80nginx /etc/rc.d/S97webrtc \
        /etc/rc.d/S97cfs_safe_monitor /etc/init.d/S97cfs_safe_monitor \
        /etc/rc.d/S98cfs_db_guard /etc/init.d/S98cfs_db_guard \
        /etc/rc.d/S99spoolman_cfs_sync /etc/init.d/S99spoolman_cfs_sync \
        /etc/rc.d/S99timelapse_recover /etc/init.d/S99timelapse_recover \
        /usr/share/klipper/klippy/extras/garbage_collection.py \
        ...
        /usr/bin/ss \
        /etc/nginx/nginx.conf /etc/rc.local /etc/init.d/moonraker; do
