...
install_camera() {
    ...
    if [ ! -s "$GO2RTC" ]; then
        newest_candidate=
        for candidate in /mnt/UDISK/helper-script.bak.*/go2rtc; do
        ...
        if [ -n "$newest_candidate" ]; then
            log_info "Restoring go2rtc from $newest_candidate"
            cp "$newest_candidate" "$GO2RTC"
        fi
    fi
    if [ ! -s "$GO2RTC" ]; then
...
"
    ...
    # Moonraker's webcam tester expects iproute2 `ss` output. The K2 image
    # only has BusyBox netstat, so install the narrow listener-format shim.
    sh "$SCRIPT_DIR/scripts/moonraker_webcam_test.sh" install || {
        log_error "Moonraker webcam-test compatibility could not be installed."
        return 1
    }
