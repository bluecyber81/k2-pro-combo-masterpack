...
& ruff check --no-cache --select F --target-version py39 -- @($pythonFiles.FullName)
if ($LASTEXITCODE -ne 0) {
...
Write-Host "K2_HELPER_VALIDATION_OK shell=$($shellFiles.Count) python=$($pythonFiles.Count) shell_tests=2"
