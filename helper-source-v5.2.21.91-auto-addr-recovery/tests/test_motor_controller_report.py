...
class MotorControllerReportTests(unittest.TestCase):
    ...
    def test_runtime_ttys_handshake_gap_is_not_a_flash_failure(self) -> None:
        status, rs485, mcu = MODULE.healthy_fixture()
        rs485["CFSs"] = []
        mcu = {
            "mcu0": {"version": ""},
        ...
        report = MODULE.analyze(
            ...
            "\n".join(
                (
                    "handshake /dev/ttyS2 fail, ret=2",
                    "handshake /dev/ttyS3 fail, ret=2",
                    "addr [85] dev [motor], update: done",
                )
            ),
            "",
        ...
        self.assertEqual(report.summary_fields["updater_failures"], "0")
        self.assertEqual(report.summary_fields["updater_handshake_gaps"], "2")
        self.assertTrue(
            any("runtime motor_ready=true" in line for line in report.lines), report.lines
        )

    def test_real_updater_failure_is_not_hidden_by_runtime_readiness(self) -> None:
        status, rs485, mcu = MODULE.healthy_fixture()
        report = MODULE.analyze(
            status,
            rs485,
            mcu,
            "send msg timeout\naddr [85] dev [motor], update: failed\n",
            "",
            "",
        )
        self.assertEqual(report.exit_code, 2)
        self.assertEqual(report.summary_fields["updater_failures"], "1")
        self.assertEqual(report.summary_fields["updater_handshake_gaps"], "0")

    def test_ttys_handshake_failure_stays_fatal_when_runtime_is_not_ready(self) -> None:
        status, rs485, mcu = MODULE.healthy_fixture()
        status["motor_control"]["motor_ready"] = False
        report = MODULE.analyze(
            status,
            rs485,
            mcu,
