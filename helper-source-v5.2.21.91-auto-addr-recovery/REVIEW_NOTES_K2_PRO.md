# v5.2.21.91-auto-addr-recovery review

## v5.2.21.91 notes

- A stock four-slot CFS print completed with CFS 1.5.0 connected for all 795
  telemetry samples, `motor_ready=true` throughout and all four slot counters
  reaching zero.
- The vendor startup files left CFS, main MCU, nozzle MCU and extruder version
  rows empty while the runtime and both X/Y controllers were healthy.
- The exact ttyS2/ttyS3 `ret=2` probe gaps are now warnings only when the live
  motor runtime is ready. Any other updater failure or `motor_ready=false`
  remains a failure.
- No flash, movement, heater, cutter or direct RS485 control was added.

## v5.2.21.90 notes

- The stock 1.1.6.7 module was reproduced with loader-ACK, unknown-function
  and second-device-table tests. The first two raised or reached invalid state,
  and the later device table was not selected.
- The reviewed payload passes all three cases and preserves the printer's
  existing ten-second CFS polling adjustment.
- Installation is restricted to exact F012/CR0CN200400C10, firmware 1.1.6.7,
  stock SHA-256 and cold standby. Restore uses the checksum-verified backup.

## v5.2.21.89 notes

- The live warning was reproduced immediately after six G-code files were
  atomically replaced. Moonraker initially returned `Metadata not available`
  for two files and indexed both successfully moments later.
- A generic JSON-RPC `-32601` is no longer labelled as a HelixPrint probe;
  only the explicit `server.helix.status` unknown-method line is counted.
- Transient G-code metadata-cache misses are excluded for arbitrary `.gcode`
  names instead of two hard-coded factory filenames.
- This revision changes Health classification only. It does not restart a
  service or expose any motion, heat, CFS, camera-power or G-code write path.

## v5.2.21.88 notes

- All live discovery and validation for this revision was non-motion work:
  no axis, extruder or CFS motor commands, no heat, no probing and no print.
- The stored mesh history alternated between full 9x9 meshes over 5..295 mm
  and local 3x3 KAMP regions. Their residual ranges describe different areas
  and must not be interpreted as temperature drift.
- Real CFS G-code showed that the total Creality estimate included transition
  overhead while the `M73` timeline could remain motion-only. Marker v2 adds
  the still-outstanding transition share at each progress point.
- Three real-file regressions covered 15, 68 and 254 transitions. All lines
  outside time metadata and markers remained byte-identical.
- `gcode_time_audit.py` is read-only and scans local files only. It has no
  Moonraker, G-code, serial, motor, heater or file-write path.
- The complete passive evidence and rollback location are documented in
  `docs/K2_PRO_PASSIVE_DIAGNOSTICS_2026-08-02.md`.

## v5.2.21.87 notes

- Live firmware 1.1.6.7 reports CFS 1.5.0, two X/Y controller images
  `mot2_023_C30-mot2_002_081`, extruder image
  `mot2_022_C30-mot2_002_081`, and `motor_ready=true`.
- The new report reads only the existing Moonraker status, `/tmp` version files,
  updater log, Klipper log and vendor config. It cannot send G-code or access a
  serial device and has no flash, calibration, reboot, address or movement path.
- Updater `send msg timeout` discovery lines are informational when the final
  devices are present and `update: done`; explicit flash/startup/handshake
  failures remain failures.
- The old 071 comment in `motor_control.cfg` is reported as harmless comment
  drift. The vendor config remains untouched because the live controllers
  consistently run 081.
- CFS 1.5.0 is now the reviewed Health baseline for this exact F012 / board
  CR0CN200400C10 firmware generation.
