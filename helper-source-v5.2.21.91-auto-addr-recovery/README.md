# Creality Helper Script - K2 Pro Combo v5.2.21.91-auto-addr-recovery build

## What changed in v5.2.21.91

- Corrects the motor-controller health classification using the reproduced
  K2 Pro 1.1.6.7 startup and a completed four-slot CFS print.
- Treats the exact `/dev/ttyS2` and `/dev/ttyS3` `ret=2` version-probe gap as
  a warning only when runtime `motor_ready=true`; real updater and runtime
  failures remain fatal.
- Reports a connected CFS 1.5.0 without a vendor updater table row as a known
  diagnostic gap. This revision does not flash or control any MCU or motor.
- Adds regression tests for the live blank MCU version table and both safe and
  unsafe handshake outcomes.

## What changed in v5.2.21.90

- Adds the regression-tested `auto_addr_wrapper.py` recovery guard for the
  exact K2 Pro F012/CR0CN200400C10 firmware 1.1.6.7 stock file.
- Fixes loader/unknown callback handling and selection of later CFS device
  tables while preserving the existing ten-second CFS poll adjustment.
- Requires exact stock and payload hashes, cold standby, an atomic backup and
  a full K2/CFS reboot. Unknown firmware or file hashes are never overwritten.
- Adds status, install and restore actions plus local regression coverage.

## What changed in v5.2.21.89

- Separates real `server.helix.status` optional-plugin probes from generic
  JSON-RPC `-32601` responses.
- Treats a transient `Metadata not available for <...gcode>` response during
  Moonraker re-indexing as informational cache behavior instead of a critical
  Health warning. Other Moonraker exceptions remain visible.
- Adds regression coverage for both classifications. No printer service,
  motion, heater, CFS, camera-power or G-code control path changed.

## What changed in v5.2.21.88

- Distinguishes full-plate meshes from local KAMP/adaptive meshes and emits a
  stable comparison key. Historical residual values are now explicitly
  comparable only when scope, grid and area match.
- Adds a read-only G-code time audit to the existing hybrid-time status. It
  detects configured-but-skipped post-processing, missing time metadata and
  CFS files whose `M73` timeline omits a large share of tool-change time.
- Upgrades the Windows hybrid wrapper to marker v2. For CFS files it combines
  Creality's existing motion timeline with the share of CFS transition time
  still ahead at each `M73` point, while keeping the validated total estimate.
- Handles existing v1 files without applying the calibrated offset twice.
- Validated the new calculation against real files with 15, 68 and 254 tool
  transitions. Strict comparisons confirmed that only `M73`, the time comment
  and estimator markers changed; motion, heaters and CFS commands stayed
  byte-identical.
- Records the passive evidence, limits and rollback path in
  `docs/K2_PRO_PASSIVE_DIAGNOSTICS_2026-08-02.md`.

## What changed in v5.2.21.87

- Adds a strictly read-only motor-controller report for the K2 Pro Combo. It
  combines current Moonraker state with Creality's generated MCU version files,
  current-boot updater log and bounded Klipper log evidence.
- Reports X/Y closed-loop controllers, the extruder controller, main/nozzle MCU,
  CFS controller, updater state, cutter feedback and key781-key797 evidence
  without exposing UUIDs or sending any G-code, RS485 or controller command.
- Distinguishes expected RS485 discovery timeouts from real flash, startup and
  handshake failures and labels the stale 071 vendor-config comment as
  documentation drift rather than a failed 081 update.
- Corrects the full Health CFS reference to the reviewed firmware 1.1.6.7 / CFS
  1.5.0 live baseline and removes the misleading preflight reference to a raw
  CFS expert-control path.
- Integrates the report into menu, CLI, Health, Preflight, installer self-tests
  and regression coverage. The new command is `--motor-controller-status`.
- Includes the full evidence-backed architecture and protocol study in
