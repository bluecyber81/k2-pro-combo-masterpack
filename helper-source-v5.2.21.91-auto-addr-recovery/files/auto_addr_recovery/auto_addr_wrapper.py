...
class AutoAddrWrapper:
    ...
    def function_code_cb(self, package):
        ...
        if ack_data is None:
            if function_code != CMD_LOADER_TO_APP:
                self.dprintf("unknown function code: %d" % function_code)
            return
        dev_table_index = None
        for i in range(len(dev_table_map_table)):
            if ack_data.dev_type == DEV_TYPE_CFS_PRO:# cfs-pro 和 mb共用一个地址表，dev_type是区分设备的，所以如果是cfs-pro设备，直接找mb的地址表即可
                ...
                dev_table_index = i
                break
        if dev_table_index is None:
            self.dprintf("no address table for device type: %d" % ack_data.dev_type)
            return
        ## 记录是否在loader的标记
    ...
    def process_all(self, eventtime):
        while True:
            # K2 Pro Combo: reduce idle CFS/RS485 poll spam. Printing already uses 10s.
            time_interval = 10.0
            if self.print_stats.state == "printing" or self.print_stats.state == "pause":
                time_interval = 10
            if self.printer.is_shutdown():
