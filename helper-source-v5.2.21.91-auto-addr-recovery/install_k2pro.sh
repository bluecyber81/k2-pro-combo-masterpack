...
validate_package() {
    ...
    test -f "$SOURCE/scripts/gcode_time_audit.py"
    ...
    test -f "$SOURCE/scripts/auto_addr_recovery.sh"
    test -f "$SOURCE/files/auto_addr_recovery/auto_addr_wrapper.py"
    test -f "$SOURCE/tests/test_auto_addr_recovery.py"
    test -f "$SOURCE/docs/K2_PRO_AUTO_ADDR_RECOVERY_2026-08-02.md"
...
PYEOF
    ...
    python3 -B "$SOURCE/scripts/gcode_time_audit.py" --selftest
    ...
    K2_HELPER_DIR="$SOURCE" sh "$SOURCE/scripts/auto_addr_recovery.sh" selftest
