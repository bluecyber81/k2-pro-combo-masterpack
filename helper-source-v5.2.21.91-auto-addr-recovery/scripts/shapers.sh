#!/bin/sh
# shapers.sh - Improved Shapers Calibrations for K2 Series printers
# Includes all K1 Max shaper macros adapted for K2 hardware
...
install_shapers() {
    ...
    cat > "$SHAPERS_CFG" << 'EOF'
# Improved Shapers Calibrations — K2 Series
# Managed by Creality Helper Script
#
# Uses the resonance_tester/accelerometer configuration already present
# in printer.cfg. Probe points and accelerometer wiring are model-specific.
#
# Macros included:
...
#   SAVE_SHAPER_CALIBRATION   - apply and save recommended shaper settings
...
gcode:
  ...
  {action_respond_info("Calibration complete. Run SAVE_SHAPER_CALIBRATION to apply with CFS auto_addr protection.")}
...
gcode:
  ...
  {action_respond_info("X axis test complete. Run SAVE_SHAPER_CALIBRATION to apply with CFS auto_addr protection.")}
...
gcode:
  ...
  {action_respond_info("Y axis test complete. Run SAVE_SHAPER_CALIBRATION to apply with CFS auto_addr protection.")}

# ── Belt tension analysis ─────────────────────────────────────────────────────
# K2 CoreXY printers have dual belts (X and Y).
...
# Compare with the target shown by the printer UI or [belt_mdl] in printer.cfg.
...
gcode:
  ...
  {action_respond_info("Compare the result with Settings > Self-check > Belt Tension or [belt_mdl] in printer.cfg.")}

  TEST_RESONANCES AXIS=X OUTPUT=raw_data NAME=belt_x FREQ_START={printer['gcode_macro BELTS_SHAPER_CALIBRATION'].min_freq} FREQ_END={printer['gcode_macro BELTS_SHAPER_CALIBRATION'].max_freq} HZ_PER_SEC={printer['gcode_macro BELTS_SHAPER_CALIBRATION'].hz_per_sec} ACCEL_PER_HZ={printer['gcode_macro BELTS_SHAPER_CALIBRATION'].accel_per_hz}

  TEST_RESONANCES AXIS=Y OUTPUT=raw_data NAME=belt_y FREQ_START={printer['gcode_macro BELTS_SHAPER_CALIBRATION'].min_freq} FREQ_END={printer['gcode_macro BELTS_SHAPER_CALIBRATION'].max_freq} HZ_PER_SEC={printer['gcode_macro BELTS_SHAPER_CALIBRATION'].hz_per_sec} ACCEL_PER_HZ={printer['gcode_macro BELTS_SHAPER_CALIBRATION'].accel_per_hz}
...
gcode:
  ...
  TEST_RESONANCES AXIS={axis} FREQ_START={freq} FREQ_END={freq + 0.1} HZ_PER_SEC=0.01 ACCEL_PER_HZ={accel} OUTPUT=raw_data

  {action_respond_info("Excitation complete.")}

# ── Apply and save recommended shapers ───────────────────────────────────────

[gcode_macro SAVE_SHAPER_CALIBRATION]
...
gcode:
  ...
  {% if 'auto_addr' in pending %}
    {action_respond_info("SAVE_CONFIG blocked because CFS auto_addr is pending. Restart Klipper/Firmware first, then rerun/save only when auto_addr is clear.")}
  {% else %}
    SAVE_CONFIG
...
EOF
    ...
    printf "%b\n" "  ${GREEN}Full calibration:${NC}   INPUT_SHAPER_CALIBRATION"
    printf "%b\n" "  ${GREEN}Per-axis:${NC}           SHAPER_CALIBRATE_X  |  SHAPER_CALIBRATE_Y"
    printf "%b\n" "  ${GREEN}Belt analysis:${NC}      BELTS_SHAPER_CALIBRATION"
    printf "%b\n" "  ${GREEN}Diagnosis:${NC}          EXCITATE_AXIS_AT_FREQ [AXIS=X] [FREQ=25] [TIME=10]"
    printf "%b\n" "  ${GREEN}Raw data:${NC}           TEST_RESONANCES_GRAPHS"
    printf "%b\n" "  ${GREEN}Apply & save:${NC}       SAVE_SHAPER_CALIBRATION (blocks CFS auto_addr SAVE_CONFIG)"
    printf "%b\n" "  ${GREEN}Restore:${NC}            RESTORE_SHAPERS"
    printf "%b\n" "  ${GREEN}Status:${NC}             SHAPER_STATUS"
    echo ""
    log_info "Recommended workflow:"
    echo "  1. Run INPUT_SHAPER_CALIBRATION"
    echo "  2. Review results in console"
    echo "  3. Run SAVE_SHAPER_CALIBRATION to save when CFS auto_addr is clear"
    echo ""
    log_info "Belt tension check: Settings -> Self-check -> Belt Tension"
    echo ""
}
...
remove_shapers() {
    ...
    printf "%b\n" "${YELLOW}WARNING: This will remove Improved Shapers Calibrations.${NC}"
