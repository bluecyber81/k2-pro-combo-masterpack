...
K2_PRO_FULL_MESH_MIN = (5.0, 5.0)
K2_PRO_FULL_MESH_MAX = (295.0, 295.0)
...
def classify_mesh_scope(mesh_min, mesh_max, rows, columns):
    """Classify a stored mesh without assuming it is comparable to every other mesh."""

    width = mesh_max[0] - mesh_min[0]
    height = mesh_max[1] - mesh_min[1]
    full_width = K2_PRO_FULL_MESH_MAX[0] - K2_PRO_FULL_MESH_MIN[0]
    full_height = K2_PRO_FULL_MESH_MAX[1] - K2_PRO_FULL_MESH_MIN[1]
    coverage = max(0.0, min(1.0, (width * height) / (full_width * full_height)))
    reaches_edges = (
        mesh_min[0] <= K2_PRO_FULL_MESH_MIN[0] + 5.0
        and mesh_min[1] <= K2_PRO_FULL_MESH_MIN[1] + 5.0
        and mesh_max[0] >= K2_PRO_FULL_MESH_MAX[0] - 5.0
        and mesh_max[1] >= K2_PRO_FULL_MESH_MAX[1] - 5.0
    )
    if reaches_edges and coverage >= 0.85 and rows >= 5 and columns >= 5:
        scope = "full_plate"
    else:
        scope = "adaptive_region"
    comparison_key = "{}:{}x{}:{:.1f},{:.1f}-{:.1f},{:.1f}".format(
        scope,
        columns,
        rows,
        mesh_min[0],
    ...
    return {
        ...
        "comparison_rule": "compare_only_matching_scope_grid_and_area",
    }
...
def analyze_payload(payload):
    ...
    scope = classify_mesh_scope(mesh_min, mesh_max, rows, columns)
...
def render_report(report):
    ...
    lines = [
        ...
        "MESH_SCOPE|INFO|scope={} coverage={:.1f}% comparison_key={}".format(
            scope["scope"],
            scope["coverage_fraction"] * 100.0,
            scope["comparison_key"],
        ),
        ...
        "NOTE|INFO|Compare residual values only when scope, grid and area match; local KAMP meshes are not full-bed flatness tests.",
