#!/bin/sh
# Creality K2 Pro Combo Helper Script - v5.2.21.62-cfs-safe-tools - firmware aware
...
print_header() {
    ...
    echo ""
    printf "%b\n" "${WHITE}======================================================${NC}"
    printf "%b\n" "${WHITE}   Creality K2 Pro Combo Helper - v5.2.21.62-cfs-safe-tools${NC}"
    printf "%b\n" "${WHITE}======================================================${NC}"
...
run_cli() {
    ...
    case "$cli_arg" in
        "")
            ...
            ;;
        --version|version)
            sed -n '2p' "$SCRIPT_DIR/helper.sh" 2>/dev/null || echo "Creality K2 Pro Combo Helper v5.2.21.62-cfs-safe-tools"
            exit 0
            ;;
        --health|health)
