#!/bin/sh
# timelapse.sh - Moonraker Timelapse for K2 Series printers
...
install_timelapse() {
    ...
    if ! is_installed "moonraker_extensions"; then
        ...
        sh "$SCRIPT_DIR/scripts/moonraker.sh" install || {
            log_error "Moonraker Extensions installation failed. Timelapse not installed."
            return 1
        }
    fi
...
PYEOF
    ...
    if [ ! -f /tmp/timelapse_extract/moonraker-timelapse-main/component/timelapse.py ] || \
       [ ! -f /tmp/timelapse_extract/moonraker-timelapse-main/klipper_macro/timelapse.cfg ]; then
        rm -rf /tmp/timelapse.zip /tmp/timelapse_extract
        log_error "Downloaded timelapse archive did not contain expected files."
        return 1
    fi
...
remove_timelapse() {
    ...
    printf "%b\n" "${YELLOW}WARNING: This will remove Moonraker Timelapse.${NC}"
