...
gcode_file_limit = int(os.environ.get(
    ...
    "16" if compact_mode else "120",
))
gcode_char_limit = int(os.environ.get(
    "CFS_SCAN_MAX_GCODE_CHARS",
    "300000" if compact_mode else "5000000",
))
gcode_time_limit = float(os.environ.get(
    "CFS_SCAN_MAX_SECONDS",
    "12" if compact_mode else "0",
))
gcode_deadline = time.monotonic() + gcode_time_limit if gcode_time_limit > 0 else None
...
def scan_gcode_file(path, base):
    ...
    try:
        ...
        with handle:
            for lineno, line in enumerate(handle, 1):
                if gcode_deadline is not None and time.monotonic() >= gcode_deadline:
                    timed_out = True
                    break
                read_chars += len(line)
...
if gcode_dir.is_dir():
    ...
    for path in gcode_files:
        ...
        file_hits, timed_out = scan_gcode_file(path, gcode_dir)
