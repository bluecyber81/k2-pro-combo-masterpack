...
install_tools() {
    ...
    if ! mkdir -p \
        "$SCRIPT_DIR/state" \
        "$SCRIPT_DIR/reports" \
        "$(dirname "$SERVICE_INIT")" \
        "$(dirname "$SERVICE_RC")"; then
        log_error "Could not create CFS Safe Tools state or service directories."
        return 1
    fi
...
case "$1" in
    ...
    mesh-history) PYTHONDONTWRITEBYTECODE=1 "$PY" -B "$WORKER" --mesh-history --history-limit "${2:-20}" ;;
    consumption) PYTHONDONTWRITEBYTECODE=1 "$PY" -B "$WORKER" --consumption --history-limit "${2:-20}" ;;
    selftest) PYTHONDONTWRITEBYTECODE=1 "$PY" -B "$WORKER" --selftest ;;
    *) echo "Usage: $0 {install|remove|start|stop|restart|status|events [limit]|gcode|mesh-history [limit]|consumption [limit]|selftest}"; exit 2 ;;
esac
