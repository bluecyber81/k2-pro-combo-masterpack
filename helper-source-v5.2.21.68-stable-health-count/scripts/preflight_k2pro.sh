...
mkdir -p "$REPORT_DIR"
{
  echo "K2 Pro Combo Helper preflight report - v5.2.21.68-stable-health-count firmware aware"
  echo "Date: $(date)"
  ...
  echo "Known good handover reference from 2026-07-07: runtime firmware=1.1.6.3, CFS/T1 firmware=1.4.2, health OK with no WARN/FAIL. Treat this as a reference, not a reason to flash or downgrade."
  echo ""
  ...
  echo "- CFS material movement can use display/Creality/CFS workflow, slicer toolchange, stock M8200/CR_BOX path, or the new CFS Expert Control helper."
...
PYEOF
  ...
  if [ -x "$SCRIPT_DIR/scripts/nozzle_camera_recover.sh" ]; then
    echo ""
    echo "== Nozzle AI camera USB/UVC compact diagnosis =="
    echo "Read-only. Offline/standby can be normal while idle; Creality starts this camera on demand for AI/flow/first-layer checks."
