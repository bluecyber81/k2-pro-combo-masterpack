...
case "$1" in
    ...
    rotate-backups)
        ...
        ;;
    *)
        echo "Usage: $0 {start|repair|boot|status|check|archive-status|rotate-backups [keep]}"
        exit 2
        ;;
esac
SHELL
...
case "$1" in
    ...
    *) echo "Usage: $0 [install|remove|repair|check|status|archive-status|rotate-backups [keep]]" ;;
esac
