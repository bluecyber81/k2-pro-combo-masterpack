...
if is_installed "klipper_gc" && [ -f /usr/share/klipper/klippy/extras/garbage_collection.py ]; then
    result_row "Klipper GC" "installed" "validated upstream runtime optimization is active"
else
    result_row "Klipper GC" "available" "small optional optimization; install uses exact hash and safe full reboot"
fi
...
echo "klipper_gc_marked=$(marked_or_no klipper_gc)"
echo "klipper_gc_module=$(exists_or_no /usr/share/klipper/klippy/extras/garbage_collection.py)"
echo ""
