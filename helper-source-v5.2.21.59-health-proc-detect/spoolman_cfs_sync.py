...
v5.2.21.58-spoolman-live-sync keeps the interactive map wizard, map
enable/disable, spool listing and one-shot sync support without exposing the
raw CFS/G-code expert-control paths from the full package, and also keeps
mapped Spoolman spool metadata aligned with live CFS slot metadata.
"""
import argparse
import json
import os
...
BUILD_LABEL = "5.2.21.58-spoolman-live-sync"
...
def run_wizard() -> None:
    ...
    mapping = {
        "enabled": True,
        "_generated_by": "k2pro-helper-v5.2.21.58-spoolman-live-sync",
