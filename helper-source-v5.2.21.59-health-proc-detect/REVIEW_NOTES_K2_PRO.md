# v5.2.21.58-spoolman-live-sync follow-up

## v5.2.21.58 notes

- Created and validated a Home Assistant long-lived token for authenticated local checks; the secret is stored outside this package in the user's DPAPI-protected local secret folder.
- Live Spoolman server on `192.168.178.43:7912` is healthy: version `0.23.1`, SQLite, automatic backups enabled.
- Official upstream Spoolman latest is `v0.24.0` as of 2026-07-07, but Home Assistant currently exposes `update.spoolman_update` as `off` with installed/latest `0.23.1-0`; no HA add-on update is currently offered by the installed add-on source.
- Added live CFS metadata synchronization to `spoolman_cfs_sync.py` for mapped T1A-T1D Spoolman spools. The daemon now keeps slot marker/material/color/remain metadata current without changing Spoolman consumption weights.
- Fixed the Spoolman PATCH format for typed extra fields: text values stay JSON-string encoded, while numeric values such as `cfs_remain_percent` are sent as plain numeric strings.
- Live cleanup applied and read back: `base_url=http://192.168.178.43:7912`, location order includes `K2 Pro CFS / Vorrat`, and T1A's stale `remainLen=32%` comment was corrected to live `remain_len=100%`.
- Validation after live install: Spoolman CFS daemon running, metadata current for four mapped slots, CFS health `OK 24 / WARN 0 / FAIL 0`, Home Assistant token test OK, and Spoolman SQLite backup created at `/config/backups/spoolman.db`.
