# Creality Helper Script - K2 Pro Combo v5.2.21.58-spoolman-live-sync build

## What changed in v5.2.21.58

- Live-tested Spoolman CFS sync improvement on `192.168.178.74`: mapped T1A-T1D Spoolman spool metadata is now refreshed from live CFS slot metadata.
- The sync daemon keeps Spoolman active-spool selection behavior from v57, but also keeps `cfs_marker`, `cfs_slot`, `cfs_filament_id`, `cfs_color`, `cfs_remain_percent`, `auto_synced`, and `source` current for mapped slots.
- Fixed typed Spoolman extra-field writing: numeric fields such as `cfs_remain_percent` are sent as numeric strings, while text fields remain JSON-string encoded as Spoolman expects.
- `--spoolman-cfs-status` now prints live CFS slot material, color, and remain percent for T1A-T1D.
- Live Spoolman cleanup applied: `base_url` set to `http://192.168.178.43:7912`, location order now includes `K2 Pro CFS / Vorrat`, and the stale T1A `remainLen=32%` comment was corrected without touching consumption weights.
