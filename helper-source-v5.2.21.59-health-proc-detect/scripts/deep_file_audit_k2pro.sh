...
PYEOF
    ...
    for f in /mnt/UDISK/printer_data/logs/moonraker.log /mnt/UDISK/printer_data/logs/klippy.log /tmp/camera_startup.log /tmp/go2rtc.log /tmp/watchdog.log; do
        [ -f "$f" ] || continue
        hits=$(tail -1000 "$f" | grep -Eic "traceback|fatal|panic|segmentation|permission denied|no such file|bad gateway|connection refused")
        if [ "$hits" -eq 0 ]; then
            line OK "$(basename "$f")" "no severe hits in last 1000 lines"
        else
            line WARN "$(basename "$f")" "severe-pattern hits=$hits; inspect below"
            tail -1000 "$f" | grep -Ei "traceback|fatal|panic|segmentation|permission denied|no such file|bad gateway|connection refused" | tail -20
        fi
    done
    echo ""
